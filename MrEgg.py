...
IMG_DIR = "C:/Users/joleon/Desktop/BurgerTime/Proyecto Ingenieria/MrEgg"
#Base12=[]
#Stairs12=[]
stairsandbase = []
FacingRight = True
...
class MrEgg(pygame.sprite.Sprite):
    frame = 0
    ...
    def __init__(self, x, y, Vx, Vy):
        ...
        for i in range(12, 36):
            img = Util_Functions.load_image("MrEgg" + str(i) + ".png", IMG_DIR, alpha=True)
    ...
    def moverightE(self):
        ...
        if self.frame > 2 * 4:
            self.frame = 0
        self.image = pygame.transform.flip(self.images[self.frame // 2], True, False)
            ...
            if (self.rect.y == Cheff.rect.y):
                if CollisionWithFloor(Base12, self):
                    if not FacingRight:
                     ...
                     if ColisionBRightE(Base12, self):
                        self.moverightE()
                        FacingRight = True
                        #self.rect.center = (self.x, self.y)
                     else:
...
def ColisionEDownE(Escaleras, self):
    for escalera in Escaleras:
        if self.rect.colliderect(escalera.rect):
            if escalera.rect.bottom - 5 >= self.rect.bottom:
